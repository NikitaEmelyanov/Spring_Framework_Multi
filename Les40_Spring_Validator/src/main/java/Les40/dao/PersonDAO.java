...
// findAny() если объект есть в списке вернуть его, если его нет - вернуть null с помощью  orElse.
    ...
    //Обертка Optional<Person> говорит о том что, в данном методе может вернуться объект, а
        // может и не вернуться. Теперь не нужна проверка на null

    //Перегруженный метод show(), который возвращает через jdbcTemplate и метод query запрос
        // на получение Email'а. Далее используется BeanPropertyRowMapper , чтобы пришедший
        // запрос направить в класс Person. Далее если объект есть в списке возвращается он,
        // если нет - null.
    }
